...
<style>
    ...
    .container { width: 95%; max-width: 1300px; margin: 50px auto; padding-bottom: 100px; }

    /* HEADER PAGE */
    .page-header { margin-bottom: 40px; text-align: left; border-left: 5px solid #0284c7; padding-left: 20px; }
    ...
    /* BUTTON ACTION */
    .btn-detail {
        background: #0f172a; color: #fff; padding: 10px 15px; border-radius: 10px; 
    ...
    .nota-header { background: #0f172a; padding: 30px; color: #fff; text-align: center; position: relative; }
    .nota-body { padding: 30px; }
    .nota-footer { padding: 20px 30px 30px; border-top: 1px dashed #e2e8f0; }
                ...
                <tbody>
                    @foreach($groupedOrders as $order)
                        <tr>
                            ...
                            <td>
                                <div style="font-weight: 800; color: #0f172a;">{{ $order->customer_name ?? auth()->user()->name }}</div>
                                <div style="font-size: 12px; color: #16a34a; font-weight: 700;"><i class="fa-brands fa-whatsapp"></i> {{ $order->customer_whatsapp ?? '-' }}</div>
                            </td>
...
<div class="modal-overlay" id="modalNota" onclick="closeNota(event)">
    <div class="nota-box" onclick="event.stopPropagation()">
        <div class="nota-header">
            ...
            <i class="fa-solid fa-circle-check" style="position: absolute; bottom: -20px; left: 50%; transform: translateX(-50%); font-size: 40px; color: #fff; background: #16a34a; border-radius: 50%; border: 5px solid #fff;"></i>
        </div>
...
<script>
    function openNota(id, status, method, amount, name, wa, details) {
        ...
        details.forEach(item => {
            itemsHtml += `
                <div class="nota-item">
                    <span>${item.qty}x ${item.product ? item.product.nama_produk : 'Produk'}</span>
